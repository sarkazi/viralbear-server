...
router.get("/findCardsFromDoneList", authMiddleware, async (req, res) => {
  try {
    ...
    let requiredCards = doneCardsFromTrello.filter((card) => {
      return (
    ...
    const defineGroupName = (card) => {
      if (card.priority && card.hasAdvance) {
        return card.list === "approve" ? "approveFirst" : "doneFirst";
      } else if (card.priority && !card.hasAdvance) {
        return card.list === "approve" ? "approveSecond" : "doneSecond";
      } else if (!card.priority && card.hasAdvance) {
        return card.list === "approve" ? "approveThird" : "doneThird";
      } else {
        return card.list === "approve" ? "approveFourth" : "doneFourth";
      }
    };

    requiredCards = requiredCards.reduce(
      (res, card) => {
        res[defineGroupName(card)].push(card);
        return res;
      },
      {
        approveFirst: [],
    ...
    const finalDoneCardsList = [
      ...requiredCards.doneFirst,
      ...requiredCards.doneSecond,
      ...requiredCards.doneThird,
      ...requiredCards.doneFourth.sort((a, b) => {
        return -1;
      }),
    ].filter((doneCard) => {
      if (role === editor && !!editor.hideForEditor) {
    ...
    const finalApprovedCardsList = [
      ...requiredCards.approveFirst,
      ...requiredCards.approveSecond,
      ...requiredCards.approveThird,
      ...requiredCards.approveFourth.sort((a, b) => {
        return -1;
      }),
    ]
      .filter((approvedCard) => {
    ...
    const apiData = {
      doneTasks: finalDoneCardsList,
      approvedTasks: finalApprovedCardsList,
    };
